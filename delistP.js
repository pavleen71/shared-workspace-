document.addEventListener('DOMContentLoaded', () => {
    ...
    fetch('http://localhost:3000/user/properties')
    ...
    delistForm.addEventListener('submit', async (event) => {
        ...
        const propertyId = document.getElementById('property-type').value;
    
        try {
          const response = await fetch(` http://localhost:3000/dproperty/${propertyId}`, {
            method: 'DELETE',
          });
